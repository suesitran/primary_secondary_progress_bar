...
class _PrimarySecondaryProgressBarPainter extends CustomPainter {
  ...
  void _drawSlider(
      ui.Canvas canvas, ui.Size size, ui.Rect sliderRect, double fontSize) {
    ...
    canvas.drawRRect(ui.RRect.fromRectAndRadius(sliderRect, progressRadius),
        sliderBackground);

    var progressWidth = sliderRect.width * progressRatio;
    canvas.drawRRect(
        ui.RRect.fromRectAndRadius(
            ui.Rect.fromLTRB(sliderRect.left, sliderRect.top, progressWidth,
                sliderRect.bottom),
            progressRadius),
        sliderProgress);
    ...
    TextPainter sliderPainter = new TextPainter(
        text: TextSpan(
            text: primaryLabel, style: TextStyle(color: primaryTextColor)),
        textDirection: TextDirection.ltr,
        textAlign: TextAlign.right,
        maxLines: 1);
    final sliderTextRect = Rect.fromLTRB(sliderRect.left + 5, sliderRect.top,
        sliderRect.right - 5, sliderRect.bottom);
    ...
    final line2Painter = new TextPainter(
        text: TextSpan(
            text: primaryIndicatorLine2,
            style:
                TextStyle(fontSize: fontSize * 0.6, color: primaryTextColor)),
        textDirection: TextDirection.ltr,
        textAlign: TextAlign.center,
        maxLines: 1)
      ..layout(minWidth: indicatorWidth);

    var actualIndicatorWidth =
        Math.max(line1Painter.width, line2Painter.width) +
            5 /* padding left */ +
            5 /* padding right*/;
    ...
    final indicatorRect = ui.Rect.fromLTRB(
        indicatorLeft, indicatorTop, indicatorRight, indicatorBottom);
    canvas.drawRRect(ui.RRect.fromRectAndRadius(indicatorRect, progressRadius),
        sliderProgress);
  ...
  void _drawSecondaryIndicator(
      ui.Canvas canvas, ui.Size size, ui.Rect sliderRect) {
    ...
    final textPainter = new TextPainter(
        text: TextSpan(
            text: secondaryLabel, style: TextStyle(color: secondaryTextColor)),
        textDirection: TextDirection.ltr,
        textAlign: textAlign,
        maxLines: 1);
    textPainter.layout();
  ...
  void draw2LinesText(ui.Canvas canvas, ui.Rect boundary,
      TextPainter line1Painter, TextPainter line2Painter) {
    ...
    // draw line 1
    canvas.save();
    canvas.translate(boundary.left + (boundary.width - line1Painter.width) / 2,
        boundary.top);
    ...
    // draw line1
    canvas.save();
    canvas.translate(boundary.left + (boundary.width - line2Painter.width) / 2,
        boundary.top);
  ...
  void drawText(ui.Canvas canvas, ui.Rect boundary, TextPainter painter,
      {TextAlign textAlign = TextAlign.center}) {
    ui.Offset offset = new ui.Offset(
        0.0, boundary.top + (boundary.height - painter.height) / 2);

    canvas.save();
    switch (textAlign) {
      case TextAlign.center:
        canvas.translate(
            boundary.left + (boundary.width - painter.width) / 2, 0.0);
        break;
